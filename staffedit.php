...
if ($result->num_rows > 0) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($stmtUpdate->execute()) {
            ...
            header("Location: staff.php?id=" . $userId);
            exit;
        } else {
            // Error in user update
            $em = "Error updating staff.";
        }
            ...
            <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $userId; ?>" onsubmit="return validatePassword();">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="rolename" name="rolename" required>
                        ...
                        // Assuming 'Admin' role exists in the roles table
                        if ($resultRoles->num_rows > 0) {
                            $rowRoles = $resultRoles->fetch_assoc();
                            echo '<option value="' . $rowRoles['roles_name'] . '" selected>' . $rowRoles['roles_name'] . '</option>';
                        }
                        ?>
                    </select>
                </div>
                <input class="btn btn-primary" name="submit" type="submit" value="Update Staff">
                <button class="btn btn-danger" onclick="location.href='staff.php';" type="button">Cancel</button>
            </form>
        </div>
    </div>
</body>
